...
export function Routes() {
  ...
  async function openSmartRoute() {
    setForm({
      name: 'Rota Inteligente - ' + new Date().toLocaleDateString('pt-BR'),
    ...
    try {
      ...
      showToast(
        err.response?.data?.message ||
          'Erro ao gerar rota inteligente. Verifique se o endereço da base está no seu perfil.',
        'error',
      )
    } finally {
      setSaving(false)
      ...
      <header className="page-header">
        ...
        <div className="page-actions">
          <input
            ...
            id="routes-search"
          />
          <button className="btn-new" onClick={openNew} style={{ background: '#64748B' }}>
            Nova Rota Manual
          </button>
          <button className="btn-new" onClick={openSmartRoute} style={{ background: '#7C3AED' }}>
      ...
      {showSmartModal && (
        <div className="modal-overlay" onClick={() => setShowSmartModal(false)}>
          ...
          >
            ...
            <p>
              O sistema irá organizar os pacotes selecionados usando o endereço da sua Base como
              ponto de partida, calculando a melhor ordem geográfica do mais próximo ao mais
              distante.
            </p>
                ...
                >
                  {unassignedPackages.length === 0 ? (
                    <p style={{ textAlign: 'center', color: '#9CA3AF', padding: '20px 0' }}>
                      Não há pacotes aguardando rota no momento.
                    </p>
                  ) : (
            ...
            <div className="modal-actions">
              <button className="btn-cancel" onClick={() => setShowSmartModal(false)}>
                Cancelar
              </button>
              <button
                className="btn-save"
        ...
        <div className="modal-overlay" onClick={() => setConfirmDelete(null)}>
          <div className="modal-card" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="confirm-text">
              Tem certeza que deseja excluir a rota <strong>{confirmDelete.name}</strong>? Os
              pacotes voltarão para o status Pendente.
            </p>
            <div className="modal-actions">
